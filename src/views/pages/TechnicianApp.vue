<template>
    ...
    <h3 class="text-3xl font-bold mt-5 dark:text-white">Maestro de Técnicos </h3>
    <div class="flex justify-end m-2">
        <Button label="Nuevo técnico" icon="pi pi-plus" severity="success" class="mr-2" @click="openNew" />
    </div>
    ...
    <DataTable ref="dt" :value="products" v-model:selection="selectedProducts" dataKey="id" :paginator="true" :rows="10"
        ...
        <Column field="User_name" header="Nombre" sortable style="min-width:12rem"></Column>
        <Column field="Name_user_comercial" header="Nombre Comercial" sortable style="min-width:12rem"></Column>
        <Column field="'Reparación de aires acondicionados'" header="Servicios" sortable style="min-width:12rem"></Column>
        <Column field="Email" header="Name" sortable style="min-width:16rem"></Column>
        <Column field="ID_rol" header="Rol" sortable style="min-width:12rem">
            <template #body="slotProps">
                <Tag :value="slotProps.data.ID_rol" :severity="getStatusLabel(slotProps.data.ID_rol)" />
            </template>
        </Column>
...
onMounted(async () => {
    ...
    await fetch(`${baseUrl}api/technician/masterTechnician`, {
        method: 'GET',
        headers: {
